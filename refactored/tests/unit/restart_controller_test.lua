...
local function controller(tree, loader, failures, sourcePaths)
    ...
    return RestartController.new({
        fs = fileSystem,
        sourcePaths = sourcePaths or { "/app/codex.lua", "/app/lib" },
...
return {
    {
        ...
        fn = function()
            ...
            local restart, written = controller({
                ["/app"] = { "codex.lua", "lib", "midi_player.lua", "monitor_text.lua" },
                ["/app/codex.lua"] = false,
                ["/app/lib"] = { "engine.lua", "nested" },
                ["/app/lib/nested"] = { "worker.lua" }
            }, function(path)
    ...
    },
    {
        ...
        fn = function()
            local restart, written = controller({
                ...
                if path == "/app/lib/broken.lua" then return nil, "unexpected symbol near end" end
                return function() end
            end)
    ...
    },
    {
        ...
        fn = function()
            ...
            Harness.falsy(valid)
            assert(validationError)
            Harness.truthy(validationError:find("/app/lib", 1, true))
            Harness.equal(0, #written)
        end
    },
    {
        ...
        fn = function()
            local restart, written, deleted = controller({
                ["/app/codex.lua"] = false
            }, function() return function() end end, { write = true }, { "/app/codex.lua" })
    ...
    },
    {
        ...
        fn = function()
            local restart, _, deleted = controller({
                ["/app/codex.lua"] = false
            }, function() return function() end end, { close = true }, { "/app/codex.lua" })
