...
local ArtifactStore = require("lib.codex.storage.artifacts")
...
local ExecuteLua = require("lib.codex.tools.execute_lua")
...
local RestartController = require("lib.codex.restart_controller")
...
local ConversationTools = require("lib.codex.tools.conversations")
local StateStore = require("lib.codex.storage.state")
local Terminal = require("lib.codex.plugins.terminal")
local ToolRegistry = require("lib.codex.tools.registry")
local TurnQueue = require("lib.codex.turn_queue")
local JsonlRecorder = require("lib.codex.storage.jsonl")
...
local function buildOptionalChatBox(config, json, submit, path, console, inputs, adapters)
    ...
    local loaded, ChatBox = pcall(require, "lib.codex.plugins.chat_box")
...
function Bootstrap.build(config)
    ...
    local restart = RestartController.new({
        fs = fileSystem,
        sourcePaths = { path("codex.lua"), path("lib") },
    ...
    local mailbox = CommandMailbox.new({
        ...
        onError = function(message) terminal:error("Host command mailbox: " .. message) end
    })
    local inputs = {}
    local adapters = {}
    ...
    local function recordConversation(record)
        ...
        if not recorded then terminal:error("Could not record conversation activity: " .. tostring(recordError)) end
    end
    ...
    local function syncConversationCatalog(result)
        ...
        if not updated then terminal:error("Could not update conversation catalog: " .. tostring(updateError)) end
    end

    local function startNewConversationLog(name)
        if conversationLogId then
            local ended, endError = conversationLog:record({ type = "lifecycle", phase = "cleared" })
            if not ended then terminal:error("Could not close the conversation log: " .. tostring(endError)) end
        end
        local newId, _, startError = conversationLog:start(nil)
        if not newId then
            terminal:error("Could not start a conversation log: " .. tostring(startError))
        ...
        if not selected then
            terminal:error("Could not select the conversation log: " .. tostring(selectError))
        ...
        if not catalogReady then
            terminal:error("Could not add conversation to catalog: " .. tostring(catalogError))
            return nil, catalogError
        end
        local saved, saveError = stateStore:save(assert(session:durableState()))
        if not saved then
            terminal:error("Could not save the conversation log identity: " .. tostring(saveError))
    ...
    local engine = ChatEngine.new({
        ...
        onWarning = function(message) terminal:error(message) end
    })
    app = App.new({
        config = config,
        runtime = runtime,
