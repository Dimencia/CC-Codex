...
.DESCRIPTION
    ...
    codex becomes a directory junction. Runtime data, artifacts, and .settings
    are left alone. The current development computer uses the compatibility
    startup.lua link; new standalone installs can copy startup/cc_codex.lua
    into their startup set instead.
...
$repositoryInput = if ([string]::IsNullOrWhiteSpace($RepositoryRoot)) {
...
$sourceApplication = Join-Path $sourceRoot 'codex'
...
if (-not (Test-Path -LiteralPath $sourceApplication -PathType Container)) { throw "Missing source directory: $sourceApplication" }
...
$entries = @(
    ...
    [pscustomobject]@{ Name = 'codex'; Kind = 'Junction'; Target = $sourceApplication }
)
